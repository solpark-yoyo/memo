...
class CAS_preprocessor(object):
    def __init__(self, model = 'SD1.5', dtype = torch.float32, device = 'cuda', scheduler = None, approx_num = 1, num_inference_steps = 50, epsilon = 1e-3, rec_num = 1):
        ...
        if model == 'SD1.5': self.pipe = StableDiffusionPipeline.from_pretrained(os.path.join(_CKPT_DIR, "stable-diffusion-v1-5"), torch_dtype = dtype, safety_checker = None, local_files_only=True).to(device)
        elif model == 'SD2.1': self.pipe = StableDiffusionPipeline.from_pretrained(os.path.join(_CKPT_DIR, "stable-diffusion-2-1-base"), torch_dtype = dtype, safety_checker = None, local_files_only=True).to(device)
        elif model == 'SD2.1v': self.pipe = StableDiffusionPipeline.from_pretrained(os.path.join(_CKPT_DIR, "stable-diffusion-2-1"), torch_dtype = dtype, safety_checker = None, local_files_only=True).to(device)
